...
func (c *ControllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if volumeAs == subpathLiteral {
		...
		if value, ok := pvcFileSystemIDMap.Load(pvName); ok && value != "" {
			...
			createFileSystemsNasRes, err := cdsNas.CreateNas(opts.SiteID, pvName, opts.StorageType, int(storageSize))
	...
	if volumeAs == fileSystemLiteral {
		...
		if deleteVolume == "true" {
			...
			if value, ok := pv.Spec.CSI.VolumeAttributes["fileSystemId"]; !ok {
				...
				log.Infof("DeleteVolume: nas, volumeID is: %s, fileSystemId is: %s", req.VolumeId, fileSystemNasID)
			}
			...
			log.Infof("DeleteVolume: nas, delete pv path succeed in NAS storage")
			...
			log.Infof("DeleteVolume: cdsNas.UnMountNas, taskID is:%s", unMountNasTaskID)
